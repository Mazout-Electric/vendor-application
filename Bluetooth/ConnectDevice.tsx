...
// import { verifySignature } from 'react-native-ecc';
...
const ConnectDevice = () => {
    ...
    // const SerialNumber = '0123299ECBC32BEC';

    const PUBLIC_KEY_HEX = [
        0x9F, 0xE8, 0xB8, 0xFA, 0x1F, 0x60, 0xBC, 0x61, 0x44, 0x7A, 0x57, 0x5E, 0x6B, 0xDA, 0xDE, 0xC9,
        0xE7, 0x9B, 0x0E, 0xBF, 0x60, 0xE6, 0x0A, 0x27, 0xA6, 0xBA, 0x9A, 0x1C, 0xAA, 0x60, 0xBE, 0x03,
        0x68, 0x13, 0xBC, 0xD3, 0x28, 0x6A, 0x2D, 0x3E, 0x65, 0xC5, 0xF6, 0x61, 0x61, 0x31, 0x1C, 0x4A,
        0x1E, 0x57, 0x69, 0x42, 0xF7, 0xEA, 0x8E, 0x46, 0xC2, 0xF3, 0x16, 0xAB, 0xDB, 0x25, 0x0A, 0xC8
    ];
    // Hex to buffer
    const publicKeyBuffer = Buffer.from(PUBLIC_KEY_HEX);
    ...
    const onServiceDiscovered = async (result: any, item: any) => {
        ...
        for (const service of services) {
            ...
            for (const characteristic of characteristics) {
                ...
                if (serviceUUID === AUTH_SERVICE_UUID) {
                    ...
                    if (characteristicUUID === BROD_SN_CHAR_UUID || characteristicUUID === BROD_SG_CHAR_UUID) {
                        try {
                            const value = await BleManager.read(item.id, serviceUUID, characteristicUUID);
                            const stringValue = byteToString(value);
                            console.log(`Read from ${characteristicUUID}:`, stringValue);

                            if (characteristicUUID === BROD_SN_CHAR_UUID) {
                                serialNumber = value;
                            } else if (characteristicUUID === BROD_SG_CHAR_UUID) {
        ...
        if (signature && serialNumber) {
            ...
            try {
                await verifySignature(signature, serialNumber);
            } catch (error) {
                console.log('Error verifying signature:', error.message);
            }
        } else {
            console.log('Signature or Serial Number is missing');
        }
    };

    // const verifySignature = async (signature: string, serialNumber: string) => {
    //     try {
    //         // Convert serialNumber to buffer
    //         const msg = Buffer.from(serialNumber, 'utf-8');

    //         // Load public key
    //         const publicKey = Buffer.from(PUBLIC_KEY_HEX);

    //         // Convert signature to buffer and validate
    ...
    //         // Convert r and s to hex format
    //         const formattedSignature = {
    //             r: r.toString('hex'),
    //             s: s.toString('hex')
    //         };

    //         // Use eccrypto to verify signature
    //         const isValid = await eccrypto.verify(publicKey, msg, formattedSignature);
    //         console.log('Signature Verification:', isValid ? 'Verified!' : 'Not Verified!');
    //     } catch (error) {
    //         console.log('Error verifying signature:', error.message);
    //     }
    // };
    ...
    // Helper functions
    const hexToUint8Array = (data: string) => {
    ...
    const uint8ArrayToHex = (data: Uint8Array) => {
    ...
    const formatHex = (maybeHex: string) => {
        let hex = maybeHex;
...
function verifySignature(signature: any, serialNumber: any) {
    throw new Error("Function not implemented.");
}

function formatHex(data: string) {
    throw new Error("Function not implemented.");
}

function assert(data: Uint8Array, arg1: string) {
    throw new Error("Function not implemented.");
}
